...
MRIdir = 'C:\Users\ICN_guest\Charité - Universitätsmedizin Berlin\Interventional Cognitive Neuromodulation - PROJECT ECoG Atlas Connectomics\data\original_data\all_fmaps';
MRIdir = 'D:\HCP_MGH_32fold_groupconnectome (Horn 2017)'
...
theta_fmaps = data_T.fmaps_niifiles(find(ifreq(:,1)))
...
for a=1:length(theta_fmaps)
%     filename = strcat(MRIdir, thet
    filename = strcat(MRIdir,'\', char(theta_fmaps(a)));
    copyfile(filename, 'theta_fmaps', 'f');
end
...
% Extract all fmaps in theta band
rh_theta_fmaps = data_T.flipped_right_fmaps(find(ifreq(:,1)))
...
% copy theta files into dir
for a=1:length(rh_theta_fmaps)
%     filename = strcat(MRIdir, thet
    filename = strcat(MRIdir,'\', char(rh_theta_fmaps(a)))
    copyfile(filename, 'theta_fmaps', 'f');
end
...
% Extract all fmaps in theta band
low_beta_fmaps = data_T.fmaps_niifiles(find(ifreq(:,2)))
...
% copy theta files into dir
for a=1:length(low_beta_fmaps)
%     filename = strcat(MRIdir, thet
    filename = strcat(MRIdir,'\', char(low_beta_fmaps(a)));
...
% Extract all fmaps in theta band
rh_gamma_fmaps = data_T.flipped_right_fmaps(find(ifreq(:,2)))

% Create new dir with these files

% copy alpha files into dir
for a=1:length(rh_gamma_fmaps)
%     filename = strcat(MRIdir, thet
    filename = strcat(MRIdir,'\', char(rh_gamma_fmaps(a)))
    copyfile(filename, 'alpha_fmaps', 'f');
end
...
% Extract all fmaps in theta band
low_beta_fmaps = data_T.fmaps_niifiles(find(ifreq(:,3)))
...
%     filename = strcat(MRIdir, thet
    filename = strcat(MRIdir,'\', char(low_beta_fmaps(a)));
    copyfile(filename, 'beta_fmaps', 'f');
end
...
% Extract all fmaps in theta band
rh_beta_fmaps = data_T.flipped_right_fmaps(find(ifreq(:,3)))

% Create new dir with these files

% copy alpha files into dir
for a=1:length(rh_beta_fmaps)
%     filename = strcat(MRIdir, thet
    filename = strcat(MRIdir,'\', char(rh_beta_fmaps(a)))
    copyfile(filename, 'beta_fmaps', 'f');
end
...
% Extract all fmaps in theta band
low_beta_fmaps = data_T.fmaps_niifiles(find(ifreq(:,6)))
...
%     filename = strcat(MRIdir, thet
    filename = strcat(MRIdir,'\', char(low_beta_fmaps(a)));
    copyfile(filename, 'gamma_fmaps', 'f');
end
...
% Extract all fmaps in theta band
rh_gamma_fmaps = data_T.flipped_right_fmaps(find(ifreq(:,6)))
...
%% original High beta functional network

mkdir high_beta_fmaps
...
% Extract all fmaps in theta band
low_beta_fmaps = data_T.fmaps_niifiles(find(ifreq(:,5)))
...
%     filename = strcat(MRIdir, thet
    filename = strcat(MRIdir,'\', char(low_beta_fmaps(a)));
    copyfile(filename, 'high_beta_fmaps', 'f');
end

%% rh high beta funcitonal network
...
% Extract all fmaps in theta band
low_beta_fmaps = data_T.flipped_right_fmaps(find(ifreq(:,5)));
...
%% rh low beta functional network


mkdir low_beta_fmaps
...
% Extract all fmaps in theta band
low_beta_fmaps = data_T.flipped_right_fmaps(find(ifreq(:,4)))
...
%     filename = strcat(MRIdir, thet
    filename = strcat(MRIdir,'\', char(low_beta_fmaps(a)));
    copyfile(filename, 'low_beta_fmaps', 'f');
end
