% This is a test for the ECoG Atlas Project

addpath C:\code\spm12
addpath D:\ECoG_Atlas\functions
addpath E:\OneDrive - Charité - Universitätsmedizin Berlin\Dokumente\PROJECT ECoG Atlas Connectomics\Liz\functions
spm('defaults','eeg')
%D=spm_eeg_load('filename_of_dataset.mat')
data_path = 'C:\Users\ICN_guest\Charité - Universitätsmedizin Berlin\Interventional Cognitive Neuromodulation - PROJECT ECoG Atlas Connectomics\data\iEEG\spm_ecog.mat';
dir_path = 'C:\Users\ICN_guest\Charité - Universitätsmedizin Berlin\Interventional Cognitive Neuromodulation - PROJECT ECoG Atlas Connectomics\data\iEEG';
% data_path = 'E:\OneDrive - Charité - Universitätsmedizin Berlin\Dokumente\PROJECT ECoG Atlas Connectomics\data\iEEG\spm_ecog.mat';
% dir_path = 'E:\OneDrive - Charité - Universitätsmedizin Berlin\Dokumente\PROJECT ECoG Atlas Connectomics\data\iEEG';
% D is the spm_ecog.mat dataset
% D=spm_eeg_load('D:\ECoG_Atlas\iEEG\iEEG\spm_ecog.mat');
D=spm_eeg_load(data_path);
%%

data = D.ftraw(0);
save('fieldtrip_filename.mat','data');

% Additionally, you may want to store the info variable that includes all 
% the important information that I have included there:
%% 
info = D.info
 
save('dataset_info.mat','info');
...
% d = dir('D:\ECoG_Atlas\iEEG\iEEG');
d = dir(dir_path);

%%
%%
if ~contains('spm_ecog.mat',{d.name})
    alldata = importdata(data_path);
    data = alldata.Data';
...
%else
 %   D=spm_eeg_load('C:\Users\ICN_guest\Downloads\iEEG\iEEG\spm_ecog.mat');
end


%% Same steps for the tf file
...
save('dataset_info_tf.mat','info');

% in tf_spm_ecog.mat the number of samples Dt.samples is the only thing
% that seems different from the spm_ecog file. Dt.nsamples = 1352

% Dt info is the most imporant thing. Let's make some plots out of all the
% info!
channels = Dt.nchannels;
trials = Dt.ntrials;
samples = Dt.nsamples;

% plot(info.ChannelPosition)
...
%% save range values - from here on all peaks are z-scored within subjects and across all data
...
%% Save .mat file as .csv
% FileData = load('tf_spm_ecog.mat');
% csvwrite('tf_spm_scog.csv', FileData.M);
